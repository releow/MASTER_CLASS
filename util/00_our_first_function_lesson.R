...
import_human_peaks <- function(human_consensus_file_path) {
  
  # List all broadPeak files in the directory
  human_peak_files <- list.files(human_consensus_file_path, full.names = TRUE, pattern = ".broadPeak")
  
  # Extract sample names from file names (e.g., novageneGFP_0.5h_REP1)
  human_sample_names <- sapply(human_peak_files, function(file) {
    str_extract(file, "novageneGFP_\\d+(\\.\\d+)?h_REP\\d+")
  })
  
  # Initialize an empty list to store filtered GRanges objects
  human_peak_list <- list()
  
  # Import, filter, and store GRanges objects
  for (i in seq_along(human_peak_files)) {
    # Import peaks as GRanges
    human_peaks <- rtracklayer::import(human_peak_files[i])
    
    # Filter to keep only seqnames starting with "chr"
    filtered_human_peaks <- human_peaks[grepl("^chr", as.character(seqnames(human_peaks)))]
    
    # Add filtered peaks to the list with a sample-specific name
    human_peak_list[[human_sample_names[i]]] <- filtered_human_peaks
  }
  
  return(human_peak_list)
}
