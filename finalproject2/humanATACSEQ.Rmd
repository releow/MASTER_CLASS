...
## Summary of non-doxycycline and doxycycline overlaps 

### Result: Almost all peaks common in dox were also present in non-dox
### Result: X dox common peaks of the X non-dox peaks overlapped (X%)
### Result: X peaks found in non-dox that were not found in dox common
### Result: X unique peaks in dox and not present in non-dox
...
Number of genes overlap between RNAseq and ATACseq non-dox peaks: `r length(human_sig_rnaseq_atac_dox)


## Summary:
### Result: 4086 non-dox peaks overlap gene promoters out of X (X%)
### Result: 2 dox peaks overlap gene promoters out of X (X%) 
### Result: 208 and 2043 genes had overlap with genes that also changed by RNAseq
...
## 3 Use DESEQ2 to find out if peaks are changing in dox and non-dox conditions
...
## 3.1 Extracting and analyzing DESEQ2 results
...
```
![DESESEQ2 ATAC peaks visualized in IGV](/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/reanna/MASTER_CLASS/finalproject2/images/igv_snapshot.png)
Result: Yeah these all look the same, which is consistent with the 58,654 common peaks between non-dox and dox samples found earlier
...
# log2foldchange histogram
ggplot(human_atac_lfc_condition, aes(x = log2FoldChange)) +
  geom_histogram(binwidth = 0.1, fill = "steelblue", color = "white") +
  labs(title = "Distribution of Human DESEQ2/ATAC log2 Fold Changes",
       x = "log2 Fold Change",
       y = "Frequency") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

# histogram of log-transformed base mean (because the values are incredibly small)
ggplot(human_atac_lfc_condition, aes(x = baseMean)) +
  geom_histogram(fill = "steelblue", color = "white") +
  labs(title = "Distribution of Base Mean",
       x = "Base Mean",
       y = "Frequency") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
...
![histogram of base mean](/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/reanna/MASTER_CLASS/finalproject2/images/humandeseq2atacbasemean.png)
# all incredibly low (could indicate low raw counts or sparse ATACseq data)
